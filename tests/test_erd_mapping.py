...
여기에는 성격이 다른 테스트가 하나 더 있다. 맨 아래의 **변환 코드 보장 조건** 둘 —
...
모델에 대한 규칙이 아니라 변환 코드가 항상 지켜야 할 조건이라 자리가 여기다.
"""
...
def test_a_multivalued_field_cannot_be_the_natural_key():
    ...
    외래키가 가리킨다. 일관성 검사(`assert_sound`)가 그것을 잡지만, 잡히기 전에 **애초에
...
# ---------------------------------------------------------------------------
# 변환 코드 보장 조건 — 입력 모델 규칙이 아니라 우리 코드가 지켜야 하는 것
# ---------------------------------------------------------------------------
...
def assert_sound(logical: dict) -> None:
    ...
    규칙(`knowledge/rules.py`)이 아니라 변환 코드의 보장 조건이다. 규칙은 모델이 지킬 일이고, 이것은
