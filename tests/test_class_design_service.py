...
def test_missing_entity_operation_is_appended_before_full_repair(monkeypatch):
    local_valid = True
    combined_calls = 0
    local_payloads: list[dict] = []

    def proposal_with_entity() -> dict:
        proposal = combined_unit_proposal()
        proposal["fragment"]["Classes"].append({
        ...
        proposal["calls"].append({
            "operationRef": "RequestRecord.record",
            "parentCallIndex": 2,
        })
        return proposal
    ...
    def fake_parse(messages, schema, **_kwargs):
        ...
        if schema is InventoryProposal:
            ...
            proposal["items"].append({
                "name": "RequestRecord",
                "kind": "Entity",
                "description": "Persisted requests",
        ...
        if schema is CombinedUnitProposal:
            combined_calls += 1
            return combined_unit_proposal() if combined_calls == 1 else proposal_with_entity()
        if schema.__name__ == "StateOwnershipRepair":
            local_payloads.append(json.loads(messages[-1]["content"]))
            return {
                "className": "RequestRecord",
                "operation": {
                    "name": "record",
                    "parameters": (
                        [] if local_valid else [{"name": "value", "type": "MissingType"}]
                    ),
                    "returnType": "void",
                    "stepRefs": ["UC1:main:2"],
                },
    ...
    locally_repaired = service.generate_class_model(index)

    assert combined_calls == 1
    assert local_payloads[0]["previousCombined"] == combined_unit_proposal()
    assert local_payloads[0]["parentChoices"] == [{
        "parentCallIndex": 2,
        "operationRef": "RequestControl.process",
    }]
    assert [item.class_name for item in locally_repaired.Classes] == [
        "RequestBoundary", "RequestControl", "RequestRecord",
    ]
    assert [call.receiver_operation_id for call in locally_repaired.Collaborations[0].calls] == [
    ...
    local_valid = False
    combined_calls = 0
    local_payloads.clear()
    fully_repaired = service.generate_class_model(index)

    assert combined_calls == 2
    assert next(
        item for item in fully_repaired.Classes if item.class_name == "RequestRecord"
    ).operations[0].name == "record"
