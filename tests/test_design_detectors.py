"""클래스 다이어그램 결정론 검출기 — 눈금이 맞는지 확인한다 (네트워크 불필요).
...
# 눈금: 심어 둔 결함을 잡는가
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("case", SEEDED, ids=lambda c: c.rule_id)
def test_seeded_defect_is_caught_by_exactly_its_own_rule(case):
    """케이스마다 자기 규칙 하나만 걸려야 한다.

    둘 이상 걸리면 어느 검출기가 잡은 것인지 알 수 없고, 그러면 "전수 통과"가 검출기
    하나의 고장을 덮는다.
    """
    found = detectors.class_diagram_findings(case.model, case.state)
...
def test_every_defect_rule_has_a_seeded_case():
    """눈금 없는 규칙이 조용히 생기는 것을 막는다.

    규칙을 추가하면서 케이스를 안 만들면, 그 규칙의 "0건"은 근거가 없는데도 다른 규칙의
    0건과 똑같이 보인다.
    """
    declared = {r.id for r in rules.rules_for(rules.CLASS_DIAGRAM, rules.DEFECT)}
    seeded = {case.rule_id for case in SEEDED}
...
# ---------------------------------------------------------------------------
def test_the_clean_control_yields_nothing():
    """대조군에서 무언가 나오면 오탐이다.

    오탐이 있으면 재생성 루프가 고칠 수 없는 지적에 예산을 태우고, 위반 수가 안 줄어
    `no_improvement`로 멈춘다.
    """
    found = detectors.class_diagram_findings(CLEAN, CLEAN_STATE)
...
# 규칙 ↔ 검출기가 맞물려 있는가
# ---------------------------------------------------------------------------
def test_every_detector_rule_names_a_detector_that_exists():
    for rule in rules.judged_by(rules.CLASS_DIAGRAM, rules.JUDGED_DETECTOR):
        assert rule.detector in detectors.CLASS_DIAGRAM_DETECTORS, rule.id
...
def test_no_rule_claims_a_validator_that_does_not_exist_yet():
    """의미 검증자는 아직 없다. 그것을 판정자로 지목한 규칙이 있으면 아무도 안 본다."""
    assert rules.judged_by(rules.CLASS_DIAGRAM, rules.JUDGED_VALIDATOR) == ()
...
@pytest.mark.parametrize(
    "prompt_name",
    ["BCE_CLASS_EXTRACTION_SYSTEM_PROMPT", "BCE_REVISION_SYSTEM_PROMPT"],
)
def test_both_prompts_carry_every_enforced_rule(prompt_name):
    """추출과 수정 프롬프트가 **강제되는 규칙 전부**를 싣는가.
    ...
    prompt = {
        "BCE_CLASS_EXTRACTION_SYSTEM_PROMPT": extractor.BCE_CLASS_EXTRACTION_SYSTEM_PROMPT,
        "BCE_REVISION_SYSTEM_PROMPT": reviser.BCE_REVISION_SYSTEM_PROMPT,
    }[prompt_name]

    for rule in rules.rules_for(rules.CLASS_DIAGRAM):
        if rule.severity in (rules.DEFECT, rules.GUIDANCE):
            assert rule.id in prompt, f"{rule.id}가 {prompt_name}에 없다"


def test_prompts_say_which_rules_are_not_rules():
    """`NON_RULE`을 생성 쪽에 그 사실 그대로 주는가.

    과적합은 판정할 때가 아니라 **쓸 때** 일어난다. "액터당 Boundary 하나"를 목표로
    알아들은 모델은 필요 없는 Boundary를 지어내거나 필요한 것을 합친다.
    """
    from app.design.services.class_diagram.extractor import (
        BCE_CLASS_EXTRACTION_SYSTEM_PROMPT as prompt,
    )

    for rule in rules.rules_for(rules.CLASS_DIAGRAM, rules.NON_RULE):
