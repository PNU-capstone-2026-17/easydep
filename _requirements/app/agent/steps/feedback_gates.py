...
from app.agent.steps.step1_requirements import classify
from app.agent.steps.step3_specifications import check_specs
from app.agent.steps.step4_diagram import check_relationships


def apply_feedback_upto(state: dict, feedback: str, up_to: str):
    """app.feedback.apply_feedback_upto 로의 지연 위임.

    app.feedback가 app.agent.* 를 임포트하므로 모듈 상단에서 직접 임포트하면 순환 참조가 된다
    (graph→feedback_gates→app.feedback→app.agent.__init__→graph). 호출 시점에 임포트해 끊는다.
    함수 내부의 `from ... import`는 호출 시 모듈 속성을 조회하므로 테스트의 monkeypatch도 반영된다.
    """
    from app.feedback import apply_feedback_upto as _impl
