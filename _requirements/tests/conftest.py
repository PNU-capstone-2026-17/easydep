...
def scripted_llm(monkeypatch):
    ...
    def _install(responses):
        ...
        monkeypatch.setattr(
            "app.agent.steps.step1_requirements.invoke_structured", fake
        )
...
def fake_bert(monkeypatch):
    ...
    def _install(fn):
        monkeypatch.setattr(
            "app.agent.steps.step1_requirements.bert_available", lambda: True
        )
        monkeypatch.setattr(
            "app.agent.steps.step1_requirements.classify_bert", fn
        )
