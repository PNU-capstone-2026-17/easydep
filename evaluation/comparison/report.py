...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "## 반복 실행 요약",
        "",
        "| 대상 | 성공 실행 | 전체 요구사항 구현 실행 | 전체 추적성 확보 실행 | 총 토큰 중앙값 (전체/성공) | 시간 중앙값 초 (전체/성공) |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for item in report.get("aggregates", []):
        lines.append(
            "| {framework} (`{arm}`) | {success} | {implemented} | {traceability} | {tokens_all} / {tokens_success} | {wall_all} / {wall_success} |".format(
    ...
    lines.extend(
        [
            ...
            "## 실행별 결과",
            "",
            "| 대상 | 반복 | 상태 | 구현 요구사항 | 필수 게이트 | 제약조건 | 추적성 | 입력/출력/총 토큰 | LLM 호출 | 실행 시간(초) |",
            "|---|---:|---|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for run in report["runs"]:
        usage = run["usage"]
        lines.append(
            "| {framework} (`{arm}`) | {repetition} | {status} | {requirements} | {gates} | {constraints} | {traceability} | {input}/{output}/{total} | {calls} | {wall} |".format(
