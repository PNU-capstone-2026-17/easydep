...
- **서빙**: FastAPI (`/analyze`, `/healthz`, 웹 UI)
...
## 구조
```
app/            FastAPI + LangGraph 앱
  config.py     .env 로드 (API_KEY/BASE_URL/MODEL, BERT/스텁 토글)
...
```bash
# 인자로 요구사항 전달 (여러 개)
python -m app.cli "Users can log in with email and password." "Respond within 2 seconds."

# 파일에서 (한 줄에 하나씩)
python -m app.cli --file reqs.txt

# 대화형 입력 (빈 줄로 종료). 추상 요구사항이면 되물으며 이어서 답변 가능
python -m app.cli

# BERT 검증 생략(빠르게)
python -m app.cli --no-bert "The system shall respond within 2 seconds."
```
...
- 구조화 출력은 `with_structured_output(method="json_schema")`(= OpenAI 네이티브 `chat.completions.parse` 경로를 langchain이 감싼 것)를 쓰고, gpt-oss가 간헐적으로 빈 `parsed`를 반환하면 자동으로 JSON 모드 폴백(`app/agent/llm.py::invoke_structured`).
- 다음 단계(2~4단계) 확장은 `app/agent/steps/step2~4`의 스텁 노드를 채우고 `app/agent/graph.py`에서 배선만 늘리면 되며, 서빙 코드는 그대로 재사용.
