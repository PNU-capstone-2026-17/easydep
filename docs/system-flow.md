...
사용자 메시지와 질문·완료·오류 카드는 MySQL의 `workspace_commands`에서 매번 복원한다.
프로세스 메모리에는 이 카드의 복사본을 저장하지 않고 진행률·미리보기 같은 실행 중 이벤트만
둔다. 명령 상태 변경 신호를 받은 브라우저는 Workspace를 다시 조회해 최종 카드를 갱신한다.

진행 이벤트는 화면에 표시해도 되는 진행률·검증 요약이다. 설계 LLM 호출의 측정 이벤트에는
...
4. LLM은 유스케이스마다 실행할 `operationId` 순서만 고른다.
5. 공용 executor가 고정 OpenAPI에서 path, method, 필수 입력과 성공 응답 schema를 읽어 업무
   API를 호출한다.
6. 통과한 HTTP case와 아직 확인하지 않은 기능 요구사항을 구분해 하나의 보고서로 합친다.
...
구현 수리 prompt에는 최근 3회의 상세 결과와 그 이전 결과의 코드 기반 묶음 요약을 넣는다.
별도 LLM으로 이력을 요약하지 않으며, 오래된 시도의 실패 종류와 전략은 버리지 않는다. 이 숫자는
전체 수리 횟수의 상한이 아니라 한 요청에서 상세 로그가 차지하는 크기를 제한하는 값이다.
