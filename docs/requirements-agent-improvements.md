...
3. **C5(에이전트 갱신)** ← **다음** — 과제 목표 1의 비어 있는 절반. 트레이스는 이미 모으고
   있고, C4가 `unexamined_rules`·`ungrounded_rule`까지 더해 놓았다. 평가 세트가 선결 과제이고,
   그게 생기면 생성 프롬프트를 KB로 옮기는 것(C1의 남은 절반)도 회귀를 잡으며 할 수 있다.
4. **C2(supervisor)** — 가장 큰 개편. 아래 트레이드오프를 감수할지가 실제 결정 포인트.
   C4가 낸 결함에 **책임 단계**를 붙여 되돌리는 층이 여기다(지금 `review_model`은 표면화만 한다).
5. C6은 병행 가능(스키마 작업), C3·C7은 C2에 딸려간다.
