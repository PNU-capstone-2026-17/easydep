...
## 13. 규칙 순위 — 외부 코퍼스 단독 프로브 (2026-07-27, 진행 중)
...
조건: 규칙 6개 × 명세 5개 × 3회, 프롬프트 판 `8a3aca627ccd`, 클라이언트 상한 90초.

| 규칙 | 항상 | 때때로 | 없음 | 흔들림 |
| --- | ---: | ---: | ---: | ---: |
| `spec.no-scope-creep` | **9** | 1 | 0 | **10%** |
| `spec.black-box-no-internal-components` | 2 | 2 | 6 | 50% |
| `spec.consequence-is-a-guarantee` | 2 | 2 | 6 | 50% |
| `spec.remerge-re-establishes-state` | 2 | 2 | 6 | 50% |
| `spec.no-hidden-branching` | 0 | 8 | 2 | **100%** |
| `spec.no-precondition-recheck` | 0 | 1 | 9 | 100% |

세 갈래로 나뉜다:

1. **`no-scope-creep`** — 10명세 중 9개가 3/3. 판정 가능한 규칙 중 압도적 1위.
2. **50% 셋** — `black-box-no-internal-components`가 §9에서도 50%였다. 다른 코퍼스·다른
   계기에서 같은 수가 나온 것이라 이 표에서 가장 믿을 만한 부분이다.
3. **`no-hidden-branching`은 잡음 발생기다** — 10명세 중 8개에서 걸리는데 **한 번도 3/3이
   아니다.** 자주 걸리는 것과 판정 가능한 것은 다르다. `no-precondition-recheck`은 반대로
   이 코퍼스에서 거의 안 걸린다.
...
도메인이 2종뿐이고 **둘 다 성격이 비슷하다** — keepass·peering 모두 2008년 `<req>` 태그가
있는 정통 SRS다. 산문에서 조동사로 캔 문서(themas·tcs·phin)는 아직 안 들어갔다. §9가 남긴
규율("도메인 여럿의 합계로만 승격·강등한다")의 문턱이 아니다. 캠페인이 계속 돌고 있다.
