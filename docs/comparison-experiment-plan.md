...
| 조건 | 방식 | 고정 버전·역할 |
|---|---|---|
| B1(선택) | 단일 LLM + CoT | 공통 산출물 계약을 받는 단일 에이전트 기준선 |
| B2 | MetaGPT + 공통 산출물 프롬프트 | 0.8.2 기반 SOP 멀티 에이전트 |
| B3 | ChatDev + 공통 산출물 프롬프트 | 1.1.6 기반 대화형 단계 시스템 |
| P | EasyDep full | 동일 요구사항만 받는 제안 시스템 |
...
세 시스템의 기본 산출물 형식이 다르므로 B2와 B3에는 요구사항 명세, 클래스·시퀀스
다이어그램, API 명세, 데이터 모델, 소스 코드, 테스트, 컨테이너와 IaC를 요구하는 동일한
공통 산출물 계약을 추가한다. EasyDep에는 같은 요구사항과 제약만 입력하고 제품 파이프라인이
산출물을 생성하게 한다. 비교기는 특정 파일 형식을 강제하지 않고 프레임워크별 어댑터가
Mermaid·PlantUML·Markdown 등 고유 산출물을 공통 의미 범주와 연결하게 한다.
...
- 고정된 요구사항 원문과 공통 산출물 계약의 SHA-256

MetaGPT와 ChatDev에는 바이트 단위로 같은 공통 산출물 프롬프트를 전달하고, EasyDep에는
동일한 요구사항 원문만 전달한다. 프레임워크가 생성한 고유 출력은 adapter로 정규화하되 내부
EasyDep 스키마를 기준군에 강요하지 않는다. 모델·endpoint가 달라지면 run별로 기록하고
“시스템+모델 묶음” 결과로만 해석한다. 특정 MetaGPT·ChatDev 버전 결과를 최신판이나
framework 전체로 일반화하지 않는다.
...
지표는 공통 산출물 커버리지, 요구 추적률, 필수 링크 recall, 교차 산출물 모순 수와 근거 없는
결정 수다. 공통 산출물 커버리지는 실제 파일 존재를, 요구 추적률과 모순 검사는 의미 연결과
정합성을 각각 측정하므로 서로 대체하지 않는다.
