...
# 리전 간 지연 (2026-07-23) — 축 하나가 다른 축의 버그를 잡았다

1차 조사에서 "찾기만 했다"로 남긴 `latency_infos` 10,890건을 담았다.

## 무엇을 잰 값인가 — 담기 전에 확인했다

`assets/cloudlatencymap.csv`는 cb-tumblebug의 `benchmark.go`가 **직접 쓴다**. 즉
벤더 SLA가 아니라 **실제로 VM을 띄워 잰 왕복 시간**이다. 그래서 '보장'이 아니라
'어느 시점의 관측'이고, **측정 시각은 원본에 없다**(DB의 `measured_at`은 적재 시각).

## 전치 채움을 의심하고 세어 봤다 — 기우였다

`benchmark.go`에 이런 줄이 있다.

```go
// Fill empty with transpose matix
```

측정 안 된 방향을 반대 방향에서 복사한다는 뜻이다. 그러면 **내가 잰 대칭성은 독립
검증이 아니다.** 세어 봤다.

```
양방향 쌍 4,851 중 정확히 같은 값     6 (0.1%)
행 98개 중 전부 대칭인 행             0 (0.0%)
```

이 스냅샷에서는 거의 안 걸렸다. **세지 않았으면 "대칭이니 양방향 측정"이라고 잘못
말할 뻔했다.**

## 물리 검증

```
대권거리 대비 상관계수 r = 0.817
거리대별 지연 중앙값이 단조 증가 (21.8 → 76.1 → 108.6 → 188.0 → … → 291.9 ms)
왕복 광속(광섬유 200,000 km/s) 위반 16 / 10,791 (0.15%)
```

## 그리고 그 16건이 **우리 리전 카탈로그의 버그**였다

불변식이 지목한 쌍을 따라가니 `alibaba eu-west-1`이 나왔다.

```
alibaba eu-west-1   이름 "UK (London)"   좌표 (53.0, -8.0)   ← 아일랜드
aws     eu-west-1   이름 "Ireland"        좌표 (53.0, -8.0)   ← 같은 좌표
```

**이름은 런던인데 좌표는 아일랜드**다. 런던↔런던이 2.9 ms인 것은 맞는 값이고, 틀린
것은 거리였다.

같은 종류가 더 있는지 188개 리전을 훑었다 — 좌표를 공유하는 지점 17개 중 이름의
도시가 다른 것이 8개인데, **7개는 오탐**이었다(Melbourne/Melbourne Australia,
Silicon Valley/California, Korea Central/Seoul 같은 표기 차이). **진짜 결함은 1건**이다.

**좌표를 고치지 않았다.** 런던의 정확한 좌표를 우리가 새로 주장하는 것이 되기 때문이다.
대신 `suspect`로 표시하고 불변식이 계속 보고하게 뒀다 — 조용히 버리면 다음 사람이
같은 것을 다시 발견한다.

## 답변

```
aws-ap-northeast-2 기준 왕복 지연 (가까운 순, 110곳 중 8곳):
       0.2 ms  aws-ap-northeast-2
       3.3 ms  azure-koreacentral · 29 km
       3.8 ms  gcp-asia-northeast3 · 26 km
       4.0 ms  tencent-ap-seoul · 29 km
       8.3 ms  azure-koreasouth · 318 km
      28.3 ms  aws-ap-northeast-3 · 553 km
```
...
# 충분한가 — 재고, 메우고, 실측했다 (2026-07-23)

프로브를 먼저 돌리면 **데이터 구멍 때문에 실패한 것을 라우팅 실패로 오독한다.**
그래서 커버리지부터 쟀다.
...
|---|---|
| 번들 | 핵심 18종 중 **16종** 답함 |
| 사이징 | **예약 IP에 aws·gcp 없음** ← 가장 많이 묻는 둘 |
| 지연 | kt·ncp·nhn·openstack 없음 (상류가 안 쟀다 — 우리가 못 메운다) |
| 이미지 | openstack만 없음 |

## 구멍 A — 예약 IP: **소스가 없어서 손으로 적었다**

기계 판독 소스를 찾았고 전부 막혔다.
...
그래서 `human-review` 근거로 **따로 담았다**(`reviewed-sizing.json`). 값이 의심스러워서가
아니라 **출처가 기계 판독이 아니라서** 산출물을 갈랐다 — 진짜 소스가 생기면 이 파일만
지우면 된다. 항목마다 사람이 확인할 곳을 적었다.

**답변이 그 사실을 말하는지도 고쳤다.** 근거 라벨만 보이면 `손 검수` → "원본에
명시됨"으로 읽혀 기계 판독 소스가 있는 것처럼 들린다.

```
aws   /24 → 251개  ⚠ **기계 판독 소스가 없어 사람이 적은 값**입니다. 확인처: …
azure /24 → 251개  근거: cb-tumblebug 네트워크 계획표        (경고 없음)
kt    /26 → "예약 IP 수를 모릅니다 — 없다는 뜻이 아니라…"      (계산 거부)
```

세 경우가 화면에서 갈린다.

## 구멍 B — AWS RDS·EKS: **메우려다 절반만 됐고, 그렇다고 적었다**

`widdix/aws-cf-templates`(Apache-2.0)를 더해 템플릿 299 → **362**, 앵커 22 → **30**.

**그래도 RDS는 15건, EKS는 2건으로 임계(20)에 못 미친다.** `MIN_SAMPLES`를 낮춰
맞출 수 있었지만 그러면 그 문턱을 둔 이유가 사라진다 — **구멍으로 남기고 coverage에
적었다.**

성격이 다른 둘을 섞는다는 것도 적었다. 공식 샘플은 서비스별 데모이고 widdix는 운영용
스택이라 CloudWatch 경보가 55.6%로 나온다 — 편향 방향이 다르다.

## 실측 — 새 축 넷

```
✓ [SZ1] /24에 몇 대?          sizing_subnet_capacity  → **251대**
✓ [SZ2] K8s 노드 최소 사양?   sizing_requirements     → "쿠버네티스 자체나 클라우드
                                                        공급자가 강제하는 규격이 아닙니다"
~ [LT1] 서울에서 가장 가까운?  cap_region_latency      → Azure Korea Central 3.3ms
✓ [IM1] arm64 이미지?         resolve_region → basic_image → ami-…(arm64)
✓ [BU1] VM 하나만 만들면 되나? bundle_for_resource
✓ [BU2] sg-default 써도 돼?   bundle_describe         → "프로덕션에서는 지양"
```

**전체 37건 중 36 통과.** SZ2에서 원본 경고가 답변까지 그대로 살아남은 것이 특히
값어치 있다.

## 남은 실패 하나와 가설

`R2`(지명→리전코드)가 실패했다. **이건 기존에 기록된 불안정**이다 — 프로브 문서에
*"모델이 도구를 건너뛰고 기억으로 답하는 일이 가끔 있음"*으로 이미 적혀 있다.

다만 **도구가 29개 → 38개로 늘었다**(cap 14 · kb 6 · cost 4 · sizing 4 · perf 3 ·
bundle 3 · 범용 4). 선택지가 늘면 선택이 어려워진다는 가설이 성립할 수 있다.
**재지 않았으므로 그렇다고 말하지 않는다** — 도구 수를 줄이거나 라우팅을 손보기 전에
반복 측정이 필요하다.
