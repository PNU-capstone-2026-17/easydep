...
## 2-0. 전수 재조사 (2026-07-21)

§5 체크리스트를 닫은 뒤, **아직 개별 확인하지 않은 20건을 현재 산출물로 다시 쟀다.**
그럴 이유가 있었다 — 앞서 손댄 항목 중 셋이 재조사에서 결론이 달랐다(D5는 모순이
아니었고, D4의 `0`은 센티널이 아니었으며, kcc-description 대상은 대부분 정확했다).

**결론: 20건 중 18건은 실재한다. 수치만 일부 다르고, 성격이 뒤집힌 것은 없다.**
다만 AWS zip이 고정 불가라 AWS 유래 수치는 감사 시점과 다르다 — 이건 결함이 아니라
§6에서 예고한 드리프트다.

| # | 재조사 결과 | 감사 | 지금 |
|---|---|---|---|
| D2 | ✅ 그대로 | 130건 | 130/130 — Inferentia·Trainium·FPGA 전부 가속기 필드 없음 |
| D3 | ✅ 그대로 | 1건 | `kt rp-48x2-rtx` 48 vCPU / **2 GiB** (형제는 450 GiB) |
| D6 | ⚠️ **수치 정정** | 2,862/2,863 (99.97%) | **89.6%** — 다만 실체는 그대로다. tencent 최저가 구간의 서로 다른 값이 `0.01` **하나뿐**이라 최저가 정렬이 그 안에서 무작위다 |
| D7 | ✅ 그대로 | 68,705/68,705 | 100% — 전 가격이 float32로 정확히 표현된다(= float32를 거쳤다) |
| D8 | ⚠️ **더 나쁨** | T4 4표기 / NA 160건 | T4 **13표기** / `NA`로 시작하는 표기 24종·151건(정확히 `NA`인 것 128건) |
| D9 | ✅ 그대로 | 73,083건 | 73,083/73,083 전부 `"node"` |
| D10 | ✅ 그대로 | 182건 | 182/182 가격 없음 (e2-micro/small/medium, f1-micro, g1-small) |
| P1 | ✅ 그대로 → **해결** | 8건 | 아래 참조 |
| P2 | ✅ 그대로 | 34,846건 | 34,846/34,846 (`Standard_E4-2ds_v5` → `azure+southindia+standard_e4-2ds_v5`) |
| P4 | ✅ 그대로 | 3패밀리 12스펙 | 3패밀리 — Eadv6·EASv4·HBrsv2에서 같은 family 안에 `acu` 있는 SKU와 없는 SKU가 섞임(428건 결측) |
| P5 | ✅ 그대로 | 62,851건 | 62,851건이 `note: null` |
| G2 | ✅ **해결됨** | 482건 | 0건 — 경로가 전부 실재한다 |
| G3 | ✅ **해결됨** | 24건 병합 | `relationshipRef` 87/87에 `target_property`가 있다 |
| G5 | ⚠️ **범위 확대** | aws 19/61건 | aws required 20·cardinality 43, **azure·gcp에도 있다**(azure 5/17, gcp 11/15) |
| G6 | ✅ 그대로 | 고립 31.8% | aws `contained_in` **0건**·고립 33.3%, **gcp도 0건**·고립 21.3% |
| K1 | ✅ 그대로 | 191/5,064 | 197/5,096 — `\p{L}` Java 방언 169건 + 원본 버그(`multiple repeat` 6건) |
| K2 | ✅ 그대로 | 1,007건 | 1,008건 (19.8%) |
| K5 | ✅ 그대로 | 13,506건 | 13,553건 **전부 True** |
| K6 | ✅ 그대로 | 42건 중 ~12건 | 38건 — 센티널(`-1`)과 진짜 하한(`-2147483648`=int32 최소, `-10000`=가중치)이 섞여 있다 |
| K8 | ⚠️ **일부 해결** | 3건 / 52건 | 비수치 9건 / `unit` 52/52 null. 다만 R4의 라벨 분리로 이들은 이제 `azure-limits-note`(짐작)라 표의 숫자와 구분된다 |

### P1 해결 — 확신에 찬 오답을 없앴다

이 저장소에서 유일하게 **사실과 반대되는 명제를 사실로 단언**하던 곳이다.

```
t1.micro  sustainedCpu = {value: true, evidence: aws-burstable-field, 신뢰도 1.0}
t2·t3·t3a·t4g = false     ← t1만 혼자 true
```

AWS API가 `BurstablePerformanceSupported: false`를 실제로 준다. t1은 T2 크레딧
모델보다 **앞선 세대**라 AWS가 그 필드에서 제외한 것인데, 파서가 이를 "상시 성능
보장"으로 옮겼다.

**뿌리는 t1이 아니라 비대칭을 못 본 것이었다.** 이 필드는 한 방향만 직접 말한다:

- `true` → "버스트다". 원본이 말한 것 → `aws-burstable-field` (stated)
- `false` → "버스트로 분류하지 않는다"까지. 거기서 "상시 보장"을 끌어내는 건 **추론**
  → `aws-non-burstable-inferred` (inferred)

t1을 이름으로 예외 처리하지 않았다 — 그건 다음 세대에서 같은 모양으로 재발한다.
GCP `IsSharedCpu`도 같은 비대칭이라 함께 나눴다. 결과적으로 29,395건이 `stated`에서
`inferred`로 바뀌고 답변에 "상시 성능은 그로부터의 추론입니다"가 붙는다.
**경고 경로(`value=false`)는 여전히 `stated`라 영향이 없다.**
...
6. ~~**다대일 접기 정책 명시**~~ — ✅ **2026-07-21 완료.** `_fold_regions`가 규칙 둘을
   명시한다: 리전 불변 필드는 그대로, 갈리는 수치는 **최소값 + `<필드>Range` 병기**.
   덤프 순서가 바뀌어도 답이 같다(first-wins는 순서에 따라 IOPS를 2배 과대 진술할
   수 있었다). 그리고 "경고 신호는 리전 불변"이라는 **가정을 검사로 바꿨다** —
   `region-invariant-signals`가 error 등급으로 매 빌드 확인한다(현재 3,221종 전부 참).
