...
> ✅ **2026-07-20 수정됨** ((마)와 함께 — 같은 병이라 한 곳에서 고쳤다).
> 신설한 `kbcommon/artifact.py`가 두 규칙을 강제한다:
> **쓰기는 검증 후 원자적으로**(실패 시 파일을 만들지 않아 기존 산출물이 살아남는다),
> **읽기는 실패를 값으로**(`(data, error)` — 예외를 던지면 호출자가 폴백할 기회를 잃는다).
> costkb는 깨진 산출물에 번들 36건으로 폴백하고 **그 사실을 응답에 밝힌다**(조용한
> 폴백은 "커버리지가 왜 좁아졌지?"를 미궁으로 만든다). perfkb는 폴백이 없으므로
> 미빌드와 동일하게 처리하되 꼬리말에서 손상과 미빌드를 구분한다.
> `is_built()`도 파일 존재가 아니라 **실제로 쓰고 있는지**를 답하도록 고쳤다.
> ZeroDivisionError 경로는 스키마의 `minItems: 1`이 쓰기 전에 걸러 자연히 닫혔다.
> 실빌드 재확인: costkb 73,083건 / perfkb 65,032건 정상, `.part` 잔여 0.
> 회귀 테스트: `tests/test_artifact_resilience.py` 9건.
...
> ✅ **2026-07-20 수정됨** (C2와 함께 — `kbcommon/artifact.py`의 `read_dataset`).
> 조인 지점(`_perf_note`)에도 try/except를 둬서, 성능 산출물이 깨져도 비용 추천은 산다.
...
3. ~~**C2 + (마)**~~ — ✅ 2026-07-20 완료. 같은 부류(산출물 손상 시 폴백 부재). 함께 처리.
