...
- `costkb`     — 이 패키지 (무엇을 살 수 있고 얼마인가)

**다른 두 KB와 다른 점**: graphkb/capacitykb는 공개 스키마에서 추출한 **규칙**이라
근거(evidence)와 신뢰도(confidence)를 레코드마다 남긴다. costkb는 **카탈로그**라 출처가
...
- `output/tumblebug-cost.json` — `costkb build`가 만든 미러 73k건. 있으면 이쪽을 쓴다.
"""
