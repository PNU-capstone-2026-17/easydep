...
| 승인된 `https-load-balanced-ingress` stable ID | CSP별 HTTPS·인증서 결합 실현 | 일반 HTTPS와 구분해 모델링됨 |
| `RESOURCE_SPEC.multiZone=true` | `loadBalancer` 앵커 | 현재 선택 규칙이며 보편적 필연성 주장은 아님 |
