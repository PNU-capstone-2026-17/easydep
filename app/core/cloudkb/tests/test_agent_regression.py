"""에이전트 회귀 — **옵트인**. 실제 모델을 호출한다.
    ...
    RUN_AGENT_TESTS=1 uv run pytest app/core/cloudkb/tests/test_agent_regression.py -v
...
**한 실행의 실패는 신호이지 판정이 아니다.** 실측(2026-07-24): 같은 날 두
실행에서 3건이 갈렸다 — 1-2a·3-11은 재시도까지 두 번 연속 실패했다가 다음
실행에서 두 번 연속 통과했다. 실패가 나면 재실행해 **일관 실패만** 회귀로
취급하고, 그때는 프로브 기대부터 의심할 것(R2·R4·H3 전례 — 셋 다 모델이
아니라 기대가 틀려 있었다).
"""
...
def _probe_ids() -> list[str]:
    ...
    """
    from app.core.cloudkb.tools.agent_probe import regression_probes

    return [p.id for p in regression_probes()]


def _results():
    from app.core.cloudkb.tools.agent_probe import regression_probes, run_probes
