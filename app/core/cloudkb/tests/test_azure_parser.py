...
from app.core.cloudkb.graphkb.parsers.azure import extract_references, parse_index
...
def build_with_table(table: dict, monkeypatch) -> Graph:
    monkeypatch.setattr(
        "app.core.cloudkb.graphkb.parsers.azure.load_reference_map", lambda _provider: table
    )
    g, _ = parse_index(load_index())
    extract_references(g, load_types(), heuristics=False)
    return g
...
def test_unknown_wrapper_makes_no_edge_but_is_reported(monkeypatch) -> None:
    ...
    """
    from app.core.cloudkb.graphkb.parsers import azure
