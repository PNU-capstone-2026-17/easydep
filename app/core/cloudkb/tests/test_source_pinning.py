...
from app.core.cloudkb.kbcommon import fetch
from app.core.cloudkb.kbcommon.sources import SOURCES, unpinnable
...
def test_unpinnable_sources_are_declared_as_such() -> None:
    ...
    여기 이유를 적어야 하고, 그 강제가 이 단언의 목적이다.
        ...
        gcp-architecture-framework
                             사이트 렌더링뿐이라 저장소·버전이 없다. **첫 HTML
                             소스**(사용자 승인 2026-07-24) — patternkb 산문
                             전용이고 재편은 최소 편수 불변식이 잡는다.

    azure-retail-prices·ibm은 **재배포 허가도 확인하지 못해** 별도 고지가 붙는다.
...
def test_graphkb_and_capacitykb_read_the_same_bicep_commit() -> None:
    ...
    """
    from app.core.cloudkb.capacitykb.parsers import azure as cap_azure
    from app.core.cloudkb.graphkb.parsers import azure as graph_azure
...
def test_both_cfn_parsers_read_the_same_zip() -> None:
    from app.core.cloudkb.capacitykb.parsers import cfn as cap_cfn
    from app.core.cloudkb.graphkb.parsers import cfn as graph_cfn
...
def test_graph_roundtrips_provenance(tmp_path) -> None:
    from app.core.cloudkb.graphkb.model import Graph, Node
