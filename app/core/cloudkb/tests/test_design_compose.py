...
from app.core.cloudkb.appkb.plan import ORIGIN_DESIGN, ORIGIN_DESIGNER, ORIGIN_INFERRED, ORIGIN_KB
from app.core.cloudkb.nim_agent.design_tools import (
    _VM_ASSUMED,
    _render_plan_text,
    compose,
    deployment_answer,
...
def test_kafka_hint_stays_deliberately_unmapped(design) -> None:
    ...
    이 결정은 유지된다(미결 + 패턴 자문)."""
    from app.core.cloudkb.nim_agent.design_tools import _ENGINE_CONCEPT
...
def test_design_can_come_from_a_file_path(tmp_path) -> None:
    ...
    """
    from app.core.cloudkb.nim_agent.design_tools import deployment_from_input
