...
| **infra-intent-plan-2026-07-31.md** | **인프라 의도 배선 계획**(실행 전) — 실물 확인으로 드러난 공백(사슬에 클라우드 자원 층이 통째로 없다·설계는 클래스/시퀀스뿐) · 앵커는 하류가 이미 말한다(kind:Deployment→k8sCluster) · 산출물 `easydep-infra-intent/v1alpha1`과 뷰 둘(design/provision) · P1~P5 · 위협 5(층 경계 미측정 포함) |
| **dep-analysis-journey-2026-07-31.md** | **← 의존성 분석 여정과 실험 전수.** 정의→**실험 23라운드 262스텝 표**→결과물 지도→조건부 4차원·술어 8부류→발견 10→CB의 자리→우리 구성·약점 5→재현→남은 갈래. 58주장 기준으로 재작성(첫 판은 41주장까지였다). 시각화는 depkb/dependency-graph.html(claims에서 재생성) |
| **resource-dependency-analysis-guide-2026-08-03.md** | **← 문외한용 안내.** "의존"의 종류·객관적 기준·Cloud-Barista의 올바른 자리·`depkb` 파이프라인·검토 체크리스트·현재 범위를 처음부터 설명 |
| **claims-reader-2026-08-03.md** | **← `claims.json` 해독표.** 필드·값·판정·evidence의 뜻을 실제 claim 119개와 실험 참조 299개로 모두 풀고, 각 `step`의 정의 코드 위치와 결과 파일을 연결 |
