...
아래는 cb-tumblebug 근거가 아니라 **우리 저장소의 문장**이거나 **우리 추론**이다.
같은 표에 섞으면 근거가 부풀려진다.

| 항목 | 출처 | 약한 지점 |
|---|---|---|
| ER 엔티티 → 영속 저장 필요 | `app/core/cloudkb/appkb/__init__.py:13` (우리 코드 주석) | 외부 근거가 아니다 |
| 유스케이스 액터 → 외부 노출 필요 | 우리 추론 | 소스에 그 문장이 없다 |
| OS·런타임은 정해져 있다 | `docs/implementation-agent.md` — 구현 에이전트가 **Spring 애플리케이션**을 생성한다 | 스택이 바뀌면 무너진다 |
| 사용자에게 지연 요구를 받는다 | 우리 추론 | `latency`가 우선순위 지표인 것까지만 명시다 |

### 3.4 추론인 것 — 소스에 그 문장은 없다

정직하게 갈라 둔다. 아래는 **우리가 구조에서 끌어낸 것**이고 근거 등급이 낮다.

| 항목 | 무엇에서 끌어냈나 | 약한 지점 |
|---|---|---|
| **형상(VM/컨테이너)** | `POST /infraDynamic`과 `POST /k8sClusterDynamic`으로 **엔드포인트가 갈린다** | cb-tumblebug은 이걸 **필드로 두지 않았다.** 사용자가 이미 정하고 온다고 전제한다 |
| **외부 노출 필요** | `NLBReq.Type`이 `PUBLIC/INTERNAL` 필수 | *"유스케이스 액터에서 유도된다"*는 **우리 시스템에 대한 우리 추론**이다 |
| **영속 저장 필요** | `dataDisk`의 `diskSize!` | ER 엔티티 → 영속 사슬은 `appkb/__init__.py:13`에 **우리가 적은 것**이지 외부 근거가 아니다 |
| **지연 요구** | `latency`가 우선순위 지표인 것은 명시 | *"사용자에게 지연 요구를 받는다"*는 추론 |
| **OS/런타임은 정해져 있다** | 구현 에이전트가 **Spring 애플리케이션**을 생성한다(`implementation-agent.md`) | 스택이 바뀌면 무너진다 |
