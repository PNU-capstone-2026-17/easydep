...
def verify_against_requirements(
    ...
    # **lowCarbonPreferred는 2026-08-02에 계약에서 빠졌다.** 계보 감사 결과 —
    # 근거가 벤더 문헌(GCP 프레임워크)과 우리 배선(2026-07-28)뿐이고 사용자 진술
    # 실물이 코퍼스에 없다(내부 0건 · PURE 13건 전부 오탐). stateless와 같은
    # 이유로 대조 분기도 함께 걷는다 — 받지 않는 값으로 서는 판정을 남겨 두면
    # 그 자리가 영영 침묵한다. 탄소 데이터(envkb)와 질의응답 축은 남아 있다.
