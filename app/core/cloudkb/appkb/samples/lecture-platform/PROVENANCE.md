...
## 이 산출물은 지금 계약을 **통과하지 못합니다** (2026-08-01)

`intake_report`를 지금 돌리면 `[4] RESOURCE_SPEC` 아래에 위반 셋이 나옵니다:
`schemaVersion`이 1(판 2가 됐습니다) · `workloads` 없음(판 2에서 필수로 신설) ·
`expectedConcurrentUsers`가 계약에 없는 칸(제로베이스 재구성에서 `scale`로 접혔습니다).

**고치지 않습니다.** 이 파일들은 2026-07-29에 에이전트가 실제로 낸 것이고, 손으로
새 모양에 맞춰 쓰면 **실물이라는 이 문서의 첫 문장이 거짓이 됩니다.** 계약이
움직이면 옛 기록이 계약을 못 지나는 것이 정상이고, 그 사실이 보이는 편이 낫습니다 —
도구가 침묵하지 않고 위반 셋을 그대로 냅니다.

씨앗 문장(`INPUT.md` · `requirements.txt` · `constraints.txt`)은 그대로이므로,
다시 돌리면 지금 계약에 맞는 한 벌이 나옵니다. 그때는 **같은 것이 나오지 않는다**는
위의 단서가 그대로 붙습니다.
