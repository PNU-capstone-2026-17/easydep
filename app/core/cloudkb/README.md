...
**리포 루트에서 돌립니다.** 이 패키지가 `app/core/cloudkb/` 아래로 옮겨진 뒤로 모든
진입점이 `python -m app.core.cloudkb.<이름>` 꼴입니다 — 옛 `python main.py`·
`python -m graphkb`는 루트에서 `No module named` 로 죽습니다.

```bash
uv sync                              # 의존성 설치
uv run python -m app.core.cloudkb.main
uv run python -m app.core.cloudkb.main --verbose      # 도구 호출·인자·결과·토큰 사용량 표시
uv run python -m app.core.cloudkb.main --max-turns 40 # 도구를 많이 부르는 질의용 (기본 20)
uv run python -m app.core.cloudkb.main --tumblebug    # cb-tumblebug MCP(라이브 축) 연결 — 기본 꺼짐
```
...
uv run python -m app.core.cloudkb.graphkb build --source tumblebug   # 벤더 중립 의존성 그래프
uv run python -m app.core.cloudkb.graphkb build --source cfn         # AWS
uv run python -m app.core.cloudkb.graphkb build --source mapping     # 벤더 간 동치 매핑
uv run python -m app.core.cloudkb.graphkb build --source azure       # Azure
uv run python -m app.core.cloudkb.graphkb build --source gcp         # GCP
uv run python -m app.core.cloudkb.capacitykb build --source cfn         # AWS 제약 (46,911건)
uv run python -m app.core.cloudkb.capacitykb build --source azure       # Azure 전체 (42,831건, 576파일·100MB)
uv run python -m app.core.cloudkb.capacitykb build --source azure-quota
uv run python -m app.core.cloudkb.capacitykb build --source gcp         # GCP (6,923건, 프로바이더 보강 포함)
uv run python -m app.core.cloudkb.capacitykb build --source aws-limits  # EBS 종류별 확정 한도 (교차 검증)

# CLI로 직접 질의도 가능
uv run python -m app.core.cloudkb.graphkb query --rank dependents --provider aws --limit 5
uv run python -m app.core.cloudkb.capacitykb query --immutable "AWS::EC2::Subnet"
uv run python -m app.core.cloudkb.costkb coverage                    # 데이터셋 경계 확인 (빌드 불필요)
uv run python -m app.core.cloudkb.costkb query --vcpu-min 4 --provider aws --region us-east-1

# costkb 미러 빌드는 extra가 필요합니다 (덤프 파서 pgdumplib)
uv sync --extra costkb
uv run python -m app.core.cloudkb.costkb build                       # → output/tumblebug-cost.json (73,083건)
uv run python -m app.core.cloudkb.costkb build --tag v0.12.25 --refresh
uv run python -m app.core.cloudkb.costkb build --rows-file rows.tsv  # pg_restore 우회 경로
```
...
```bash
uv run python -m app.core.cloudkb.kbcommon verify
```
...
```bash
uv run python -m app.core.cloudkb.tools.build_graph_explorer    # 굽고 localhost로 띄우고 브라우저를 연다
```
...
- **번들 데이터셋(기본, 빌드 불필요)**: `costkb/specs.json`의 큐레이션된 AWS/GCP/Azure
  ...
  경계입니다.
- **미러 빌드(권장)**: `python -m app.core.cloudkb.costkb build`를 한 번 돌리면 `output/tumblebug-cost.json`
  (73,083건 · vCPU 최대 896 · 메모리 최대 32 TB)이 생기고 도구가 자동으로 이쪽을 씁니다.
  여전히 서버·자격증명은 필요 없습니다. 커버리지: `uv run python -m app.core.cloudkb.costkb coverage`
...
```bash
uv run python -m app.core.cloudkb.main --tumblebug        # 또는 NIM_AGENT_TUMBLEBUG=1
```
...
```bash
python -m app.core.cloudkb.tools.agent_probe --list       # 질의집 + 도구 커버리지 (키 불필요)
python -m app.core.cloudkb.tools.agent_probe              # 영어 31건 (기본 — 대상 언어)
python -m app.core.cloudkb.tools.agent_probe --lang ko    # 한국어 68건
python -m app.core.cloudkb.tools.agent_probe --repeat 5   # 흔들림을 통과율로
python -m app.core.cloudkb.tools.agent_probe --tool-output 0   # 도구 결과 숨기기
python -m app.core.cloudkb.tools.agent_probe --answer 0        # 답변을 자르지 않기
RUN_AGENT_TESTS=1 uv run pytest app/core/cloudkb/tests/test_agent_regression.py
```
