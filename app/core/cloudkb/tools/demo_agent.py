"""LLM 에이전트 시연 — **무엇을 부르고 무엇을 받아 그렇게 답했는지**를 다 보여 준다.
    ...
    python -m app.core.cloudkb.tools.demo_agent --list          # 질문 목차
    python -m app.core.cloudkb.tools.demo_agent --pause         # 발표용(Enter로 한 문항씩)
    python -m app.core.cloudkb.tools.demo_agent --only 1,5,7    # 골라서
    python -m app.core.cloudkb.tools.demo_agent --full          # 도구 결과를 안 자름
    python -m app.core.cloudkb.tools.demo_agent --ask "..."     # 즉석 질문 하나
    python -m app.core.cloudkb.tools.demo_agent --save out.json # 기록 저장
