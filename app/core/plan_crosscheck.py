...
첫째는 규율이다. 검사 규칙(`Constraint.rule`)은 산문이고
(`"ALB는 서로 다른 AZ의 서브넷 ≥2"`), 기계가 위반을 단정하려면 규칙을 여기
다시 코드로 적어야 하는데 그러면 사본이 둘이 된다.

**둘째는 정보 부재이고, 실은 이쪽이 더 크다**(2026-08-01 실측). 실측 규칙
여덟 중 **일곱**이 계획에 없는 정보를 요구한다:

    가용영역   배치 조건·카디널리티 4건 — 계획의 `placement`는 `subnet`·`vnet`
               같은 **컨테인먼트**이지 AZ가 아니다
    존·속성    쌍 호환 2건 — 디스크와 인스턴스의 zone 일치 같은 것
    시점       수명 조건 1건 — 계획에 시간축이 없다

기계로 볼 수 있는 것은 `이름 조건` 하나뿐이다(azure `GatewaySubnet`).
**"판정하지 않기로 했다"와 "판정할 수 없다"를 섞어 적지 않는다** — 앞의 판이
그렇게 적어 놓아, 계획에 칸을 열면 닫히는 일곱 건이 원칙 뒤에 숨어 있었다.
...
#: 규율 때문에 판정하지 않는 것이다.
#:
#: 이 표가 있어야 "안 하기로 했다"와 "못 한다"가 갈린다. 여덟 중 일곱이
#: 뒤쪽이고, 전부 **계획의 자료 모델에 칸이 없어서**다 — 고칠 수 있는 자리라는
#: 뜻이기도 하다(2026-08-01 실측).
_WHAT_JUDGING_NEEDS: dict[str, str] = {
    "배치 조건": "자원의 가용영역(AZ)",
    "카디널리티": "자원의 개수와 가용영역(AZ)",
    "쌍 호환": "두 자원의 존·SKU 같은 속성",
    "수명 조건": "시간축(생성 시점과 그 이후를 가르는 자리)",
    # `이름 조건`은 여기 없다 — 노드 이름은 계획이 이미 담고 있다.
}
...
def crosscheck(plan: DeploymentPlan, csp: str, region: str = "-") -> Crosscheck:
    ...
    for check in provision["checks"]:
        ...
        # 사실이고, 뒤엣것은 계획에 칸을 열면 닫힌다.
        missing = _WHAT_JUDGING_NEEDS.get(check["kind"], "")
        why = (f"계획에 {missing}이(가) 없어 기계로는 판정할 수 없다"
               if missing else
               "규칙이 산문이라 여기서 판정하지 않는다 — 코드로 다시 적으면 "
               "사본이 둘이 된다")
        findings.append(Finding(
            UNCHECKED_RULE, f'{check["subject"]}→{check["object"]}',
            f"계획의 관측: {seen or '해당 자원 없음'} — {why}",
