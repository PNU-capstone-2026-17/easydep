...
def import_stage_content(app_id: str, stage: str, request: ImportRequest) -> JSONResponse:
    ...
    if stage == "deployment_diagram" and (
        not isinstance(request.content, dict)
        or request.content.get("schemaVersion") != "easydep-deployment-diagram"
    ):
        raise HTTPException(
            status_code=409,
            detail="Import a WorkloadGraph deployment bundle generated by the deployment-diagram stage.",
        )
