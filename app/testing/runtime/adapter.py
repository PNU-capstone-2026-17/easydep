"""Application-test boundary for generated implementation outputs."""
...
def _compile_owner_diagnostic(
    ...
    if not any(marker in output.lower() for marker in _COMPILE_FAILURE_MARKERS):
        return None
...
class TestingAdapter:
    """Run generated-application tests without the legacy orchestration graph."""
    ...
    def _unit_tests(
        self, repository: Path, runtime_contract: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def _run_in_member_runner(
        ...
        for line in reversed(completed.stdout.splitlines()):
            ...
            if isinstance(result, dict) and "passed" in result:
                ...
                return result
        raise RuntimeError("Linux testing runner returned no structured test result")
    ...
    def _diagnostics(
        ...
        ) or ("strategyselectionexception" in output and "classnotfoundexception" in output):
            code = "APP-DB-001"
        else:
