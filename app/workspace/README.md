...
검증에서 발견한 기술 결함의 수리는 모드와 무관하게 백엔드가 자동으로 `delegate_repair`를
이어 실행한다. 따라서 사용자가 수리 버튼을 반복해서 누르지 않는다. 배포 대상 선택이나
요구사항의 뜻처럼 사용자의 결정이 필요한 질문, 그리고 서로 다른 수리 전략이 같은 실패를
반복해 새 정보가 필요한 경우에만 `AWAITING_INPUT`으로 남는다. 자동 모드는 그 밖의 “다음
단계 진행”과 이미 승인된 실행 선택만 대신 누른다.
