"""Complete provider-native templates derived from DeploymentPlan.

This module reuses only the audited provider primitive catalogue. It never calls a
topology-family builder and
never infers workloads, persistence owners, protocols, or database engines.
"""
    ...
    for binding in deployment_plan.get("runtimeBindings") or []:
        ...
        if resolved.get("kind") == "endpointEnvironment":
            ...
            if strategy == "containerDns":
                ...
                resolved["endpointHost"] = (
                    compute_details.get(target_compute) or {}
                ).get("privateIp")
        ...
        runtime_bindings_by_workload[str(binding.get("workloadRef") or "")].append(
            resolved
        )
    ...
    for compute_id, compute in sorted(computes.items()):
        ...
        if managed:
            ...
            if provider != "azure":
                template.reference(
                    ...
                    consumer_path=(
                        "launch_template.id"
                        if provider == "aws"
                        else "version.instance_template"
                    ),
            ...
            template.add_node(
                ...
                attributes={
                    ...
                    "privateIp": str(ipaddress.ip_network(
        ...
        if identity_id:
            ...
            template.reference(
                ...
                producer_attribute={"aws": "name", "azure": "id", "gcp": "email"}[
                    provider
                ],
        ...
        details[compute_id] = {
            ...
            "zones": zones,
            "privateIp": (
                template.nodes[compute_node]["attributes"].get("privateIp")
                if not managed
                else None
            ),
        }
...
def build_complete_provider_template(
    ...
    *,
    ...
    for binding in deployment_plan.get("runtimeBindings") or []:
        ...
        owner = (
            (compute_details.get(source_compute) or {}).get("template")
            or (compute_details.get(source_compute) or {}).get("compute")
        )
            ...
            *copy.deepcopy(deployment_plan.get("derivations") or []),
            {
                ...
                "sourceRefs": [
                    f"project-policy:{normalized_provider}.complete-provider-template"
                ],
            },
        ],
    }
...
def validate_complete_provider_template(plan: dict[str, Any]) -> None:
    ...
    try:
        network_range = ipaddress.ip_network(network_cidr) if network_cidr else None
        subnet_ranges = [
            (
                str(node.get("id") or ""),
    ...
    subnet_by_logical_ref = {
        ...
        for (subnet_id, subnet_range), node in zip(
            ...
            [
                item
                for item in nodes
                if item.get("providerPrimitiveKind") == "subnet"
            ],
        )
    ...
    reference_pairs = {
        (item["consumerRef"], item["producerRef"])
        for item in plan.get("references") or []
    }
    for compute_id in set(placements.values()):
        ...
        has_filter = any((owner, filter_id) in reference_pairs for owner in network_owners) or any(
            (owner, f"network-tag-{compute_id}") in reference_pairs
            for owner in network_owners
        ) or (
