...
> 둘이 어긋나면 그쪽이 이긴다. 여기 적힌 축·개수는 낡았을 수 있다.
...
### 과제가 하려는 것 — 그리고 이 창고의 자리 ★

**먼저 과제 원문입니다**(`document/research.md`). 이 저장소가 무엇을 위해 있는지는
거기서 나오고, 이 문서가 정하는 것이 아닙니다.

> 클라우드 환경에 기반하여 클라우드 리소스 및 개발 단계별 산출물을 **생성, 배포,
> 실행**할 수 있는 **멀티 AI 에이전트 기반 클라우드 네이티브 애플리케이션 개발 지원
> 기술**
...
| | 목표 | 이 창고가 대는 것 |
|---|---|---|
| ① | 요구사항 기반 산출물 **검증** + 사용자 피드백을 위한 에이전트 갱신 | 계획을 요구와 대조해 판정문을 낸다(11장) |
| ② | 클라우드 환경·리소스 특성(**성능·비용·용량·의존성**)을 고려한 **가이드라인** | 창립 4축이 그 네 단어 그대로다(3장) |
| ③ | 에이전트 간 연계를 고려한 **단계별 산출물 생성** | 설계 산출물 → 배포 계획·다이어그램(11장) |

그리고 과제가 적은 **문제 셋** 중 첫째가 환각입니다 — *"AI 에이전트가 사용자의
의도를 정확하게 반영하지 못하는 환각 현상으로 인해 요구사항에 부합하지 않는 클라우드
리소스 환경과 서비스 기능을 산출물에 포함시킬 수 있다."*

### 그래서 이 창고가 지키는 규율

> **확신에 찬 오답을 내지 않는다.**

이건 **과제의 주제가 아니라 이 지식베이스의 설계 원칙**입니다 — 문제 ①에 대한 우리
쪽 답이고, 목표 ①~③을 **믿을 수 있게 만드는 수단**입니다. 산출물이 아무리 잘
나와도 그 안의 값이 지어낸 것이면 검증도 가이드라인도 의미가 없기 때문입니다.

> **한동안 이 문서가 이 한 줄을 "이 프로젝트가 막으려는 단 하나"라고 적어 뒀습니다.**
> 그건 과장이었습니다 — 원문에는 "확신에 찬 오답"이라는 말이 없고, 환각은 문제 셋 중
> **하나**입니다. 수단을 목적으로 적으면 **"그래서 무엇을 만들어 주는데?"**에 답하지
> 못하게 됩니다(2026-07-28 정정).

아래 장들의 설계 결정은 이 규율에서 나오지만, **규율이 목적은 아닙니다.** 목적은
개발자의 의사결정 부담을 줄이는 산출물이고, 규율은 그 산출물을 믿을 수 있게 합니다.
...
포장은 `python -m app.core.cloudkb.kbcommon pack <파일>`이 합니다. 이 명령에는 **거부 목록**이
...
`python -m app.core.cloudkb.kbcommon verify`. 단방향 규약 때문에 어느 빌드에도 넣을 수 없어서,
...
재배포를 금지하므로: 빌드는 **로컬 전용**(`python -m app.core.cloudkb.costkb build-aws-managed`,
...
```bash
# 창립 4축
python -m app.core.cloudkb.graphkb build --source tumblebug   # core 13종 (+cfn/azure/gcp/mapping/svcmap …)
python -m app.core.cloudkb.capacitykb build --source cfn      # (+azure/gcp/aws-tf/aws-conditional …)
python -m app.core.cloudkb.costkb build                       # 미러 사본 (+build-azure-managed 등)
python -m app.core.cloudkb.perfkb build                       # 성능 신호 + 보강 소스 3종

# 자란 축
python -m app.core.cloudkb.bundlekb build … ; python -m app.core.cloudkb.sizingkb build … ; python -m app.core.cloudkb.envkb build …
python -m app.core.cloudkb.patternkb build … ; # 소스별 하위 명령은 각 패키지 --help가 진실

# 축 사이 검증과 포장
python -m app.core.cloudkb.kbcommon verify
python -m app.core.cloudkb.kbcommon pack <산출물.json>        # data/*.gz 갱신 (금지 목록 내장)
```
...
```python
from app.core.cloudkb.capacitykb import agent_api as cap
from app.core.cloudkb.graphkb   import agent_api as graph
from app.core.cloudkb.costkb    import agent_api as cost
from app.core.cloudkb.perfkb    import agent_api as perf
```
...
from app.core.cloudkb.appkb import diagram, verify
