...
구조화 출력이 `rule_id`를 요구하니(`schemas.RuleFinding`) 대조할 수 있다. 지식베이스에
없는 id를 댄 지적은 버리고, 버렸다는 사실을 저하로 남긴다. `app/deployment`가 "도구
...
from app.requirements.schemas import RuleFinding
