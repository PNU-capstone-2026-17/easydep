"""PIPELINE batch 실행과 current artifact 저장을 조율하는 canonical runner 경계다.
...
# app/requirements/orchestration/runner.py 에서 저장소 루트까지는 네 단계 위다.
...
def load_state(run_dir: str | Path) -> AgentState:
...
def _run_stages(state: AgentState, to_run: tuple[stages.Stage, ...]) -> list[str]:
...
def _rerun_from(state: AgentState, owner: str) -> list[str]:
...
def _summarize(state: AgentState) -> dict[str, object]:
...
def persist_run(
    ...
    traceability_verdicts: list[dict[str, object]] | None = None,
