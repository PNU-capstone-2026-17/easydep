...
const parseArgs = argv => program
  ...
  .option('-l, --lang [lang]', 'Output language (Java only)', options, 'java')
  .option('-o, --out [path]', 'Output path. When not given output is printed to console.')
  .option('-p, --package [name]', 'Base package for generated Java source.')
  .on('--help', () => {
    ...
    print('');
    print('Examples:');
    print('  $ puml2code -i input.puml -l java -p com.example.bce');
    print('  $ puml2code -h');
...
const execute = async (argv = process.argv, printer = console.log) => { // eslint-disable-line no-console
  ...
  try {
    ...
    const output = await puml.generate(args.lang, { basePackage: args.package });
    if (args.out) {
