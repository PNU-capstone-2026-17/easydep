...
class PlantUmlToCode {
  ...
  static _registerHandlebarsHelpers() {
    ...
    Handlebars.registerHelper(
      'javaEntityMethodBody',
      (owner, method) => new Handlebars.SafeString(owner.getJavaEntityMethodBody(method)),
    );
  }
   ...
   * @private
   */
  async _toCode(data, lang, options = {}) {
    ...
    // Templates are bundled and fixed; only the parser-created model instances
    // vary. Modern Handlebars blocks their prototype getters by default, which
    // would render empty Java declarations. Permit methods only at this trusted
    // model/template boundary (never for a user-supplied template).
    const renderOptions = {
    ...
    aUMLBlocks.forEach((project) => {
      project.getClasses().forEach((element) => {
        ...
        files[`${element.getFullName()}.${extension}`] = template(element, renderOptions);
      });
    });
    return files;
  }
