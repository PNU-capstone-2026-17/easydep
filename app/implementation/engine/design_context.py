...
def _detect_stock_purchase_e2e_design_gaps_legacy(
    ...
    """Find observable contract gaps that would make an E2E test fictional."""
    package_path = spec.base_package.replace(".", "/")
    package_root = run_root / "application" / "src" / "main" / "java" / package_path
    gaps: list[dict[str, str]] = []
    scan_roots = [package_root / "application" / "impl", package_root / "adapter" / "in" / "web"]
    for root in scan_roots:
        for path in sorted(root.glob("*.java")) if root.is_dir() else []:
                    ...
                    gaps.append(
                        {
                            ...
                            "requiredAction": "Resolve this behavior in the system design contract before generating an E2E acceptance test.",
                        }
                    )

    control_path = package_root / "bce" / "StockPurchaseController.java"
    control = _read(control_path)
    purchase_request = _read(package_root / "api" / "model" / "PurchaseRequest.java")
    suggestion_request = _read(package_root / "api" / "model" / "SuggestionRequest.java")
    if purchase_request and not re.search(
        r"\bPurchaseRecord\s+startPurchase\s*\(\s*String\s+\w+\s*\)", control
    ):
        gaps.append(
            {
                "code": "PURCHASE_COMMAND_RESULT_PORT_MISSING",
                "source": control_path.relative_to(run_root).as_posix(),
                "evidence": "The Control has no startPurchase(String) operation returning PurchaseRecord while the API defines PurchaseRequest.siteName.",
                "requiredAction": "Add a design-level purchase command/result port and map the OpenAPI request/result to it.",
            }
        )
    if not re.search(
        r"\bPortfolio\s+updatePortfolio\s*\(\s*PurchaseRecord\s+\w+\s*\)",
        control,
    ):
        gaps.append(
            {
                "code": "PORTFOLIO_UPDATE_INPUT_MISSING",
                "source": control_path.relative_to(run_root).as_posix(),
                "evidence": "The Control has no updatePortfolio(PurchaseRecord) operation returning the updated Portfolio.",
                "requiredAction": "Specify how a completed PurchaseRecord becomes a Holding and is persisted before the portfolio is displayed.",
            }
        )
    if suggestion_request and not re.search(
        r"\bboolean\s+submitSuggestion\s*\(\s*String\s+\w+\s*\)", control
    ):
        gaps.append(
            {
                "code": "SUGGESTION_COMMAND_INPUT_MISSING",
                "source": control_path.relative_to(run_root).as_posix(),
                "evidence": "The Control has no submitSuggestion(String) operation for SuggestionRequest.siteName.",
                "requiredAction": "Add an input-bearing suggestion command or revise the API/sequence ownership of suggestion state.",
            }
        )
    service_root = package_root / "application" / "impl"
    service_sources = "\n".join(
        path.read_text(encoding="utf-8")
        for path in sorted(service_root.glob("*.java"))
    ) if service_root.is_dir() else ""
    repository_root = package_root / "persistence" / "repository"
    if any(repository_root.glob("*Repository.java")) and not re.search(
        r"\bpersistence\.repository\b|\b\w+Repository\b|\bStockPurchasePersistenceGateway\b",
        service_sources,
    ):
        gaps.append(
            {
                "code": "PERSISTENCE_WRITE_PATH_MISSING",
                "source": service_root.relative_to(run_root).as_posix(),
                "evidence": "Spring Data repositories exist, but no application service consumes a repository or persistence output port.",
                "requiredAction": "Define and wire persistence output ports/adapters for PurchaseRecord, Holding, Portfolio, LogEntry, and WebSitePreference writes before E2E generation.",
            }
        )

    stock_path = service_root / "StockPurchaseControllerService.java"
    stock_source = _read(stock_path)
    start_body = re.search(
        r"public\s+PurchaseRecord\s+startPurchase\s*\([^)]*\)\s*\{(?P<body>.*?)(?=\n\s*@Override|\Z)",
        stock_source,
        re.DOTALL,
    )
    if start_body and "updatePortfolio(" not in start_body.group("body"):
        gaps.append(
            {
                "code": "PURCHASE_SUCCESS_PORTFOLIO_PATH_MISSING",
                "source": stock_path.relative_to(run_root).as_posix(),
                "evidence": "startPurchase returns the intercepted PurchaseRecord without invoking updatePortfolio for a completed purchase.",
                "requiredAction": "Specify and implement the completed-purchase transition through Holding creation, portfolio update, and persistence.",
            }
        )

    web_path = service_root / "WebConnectionManagerService.java"
    web_source = _read(web_path)
    if 'String response = "ACK"' in web_source and "TradingSite" not in web_source:
        gaps.append(
            {
                "code": "EXTERNAL_OUTCOME_ADAPTER_MISSING",
                "source": web_path.relative_to(run_root).as_posix(),
                "evidence": "The production WebConnectionManager hard-codes ACK and exposes no external gateway/test-profile adapter for failure, delay, or trade details.",
                "requiredAction": "Define an outbound trading-site gateway and production/test adapters capable of success, connection failure, delayed, and missing-information outcomes.",
            }
        )
    gateway_contracts = package_root / "bce"
    gateway_outputs = {
        "StockPurchasePersistenceGateway": package_root / "adapter" / "out" / "persistence" / "StockPurchasePersistenceGatewayAdapter.java",
        "TradingSiteGateway": package_root / "adapter" / "out" / "trading" / "InMemoryTradingSiteGatewayAdapter.java",
    }
    for gateway, adapter_path in gateway_outputs.items():
        if (gateway_contracts / f"{gateway}.java").is_file() and not adapter_path.is_file():
            gaps.append(
                {
                    "code": "GATEWAY_ADAPTER_MISSING",
                    "source": (gateway_contracts / f"{gateway}.java").relative_to(run_root).as_posix(),
                    "evidence": f"{gateway} exists but its contracted outbound adapter has not been generated.",
                    "requiredAction": "Run and verify the outbound Gateway adapter phase before E2E generation.",
                }
            )
    return gaps
