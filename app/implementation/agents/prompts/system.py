...
For Control tasks, generated BCE Controls are application port interfaces: implement the matching Control interface and inject only the explicitly embedded Gateway or Control ports through the constructor. Never infer a repository, RepositoryPort, Gateway, package, or persistence adapter from an Entity name. If an embedded contract does not provide a persistence port, finish without adding an import for one and keep only contract-expressible behavior compilable. For persistence tasks, follow the task-specific JPA, repository, mapper, and schema rules. For API adapter tasks, implement the exact generated OpenAPI interface and delegate only through existing application Control ports. Never edit generated BCE or OpenAPI contracts.
