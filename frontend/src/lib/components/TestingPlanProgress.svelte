<script lang="ts">
  ...
  let completed = $derived(plans.filter((plan) => ['PASS', 'REUSED'].includes(plan.status)).length);
  function label(plan: TestingRunView['plans'][number]) {
    if (plan.status === 'RUNNING') return (plan.attempt ?? 1) > 1 ? '수정 중' : '생성 중';
    return ({ PASS: '생성 완료', REUSED: '기존 계획 재사용', FAIL: '생성 실패' } as Record<string, string>)[plan.status] ?? '대기';
  }
</script>
      ...
      {#each plans as plan (plan.workflowId)}
        <li class="rounded-lg px-2 py-2 text-xs {plan.status === 'RUNNING' ? 'bg-[#f6f0df]' : 'bg-[#f7f8f5]'}">
          <div class="flex items-start justify-between gap-3">
            ...
            <span class="shrink-0 font-medium {plan.status === 'FAIL' ? 'text-[#a24037]' : plan.status === 'RUNNING' ? 'text-[#986f22]' : plan.status === 'PENDING' ? 'text-[#777970]' : 'text-[#2d7354]'}">{label(plan)}</span>
          </div>
