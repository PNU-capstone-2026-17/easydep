(() => {
  ...
  async function loadApp() {
    ...
    try {
      ...
      setNotice(
        "appMessage",
        ready ? "구현에 필요한 클래스 다이어그램과 OpenAPI 명세가 준비되었습니다." : "클래스 다이어그램과 OpenAPI 명세를 설계 화면에서 먼저 완료하세요.",
        ready ? "success" : ""
      );
  ...
  function renderImplementation(job) {
    ...
    const canCancel = ["QUEUED", "PLANNING", "RUNNING", "AWAITING_APPROVAL"].includes(job.status);
    ...
    if (job.status === "COMPLETED") {
      ...
      setNotice("implementationMessage", job.error || "구현을 완료하지 못했습니다. 상세 결과를 확인하세요.", "error");
    } else {
